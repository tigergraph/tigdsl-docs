...
The *first* time you call an algorithm on a particular graph and with a particular set of schema elements (vertex types, edge types, and attributes if appropriate),
it goes through a short just-in-time-compilation process to optimize the execution plan for the query.

If you run the same algorithm with the same schema elements (regardless of modification to other parameters), it will skip the compilation and immediately run the algorithm query.
