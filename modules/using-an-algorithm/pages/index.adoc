...
Database versions 3.8 and higher offer the library in an additional format, as packaged template queries.
...
Import all queries in a package with `IMPORT PACKAGE`.
...
`SHOW TEMPLATE QUERY -r "GDBMS_ALGO.community.tri.*"` matches all Community template queries starting with the string `tri`.
