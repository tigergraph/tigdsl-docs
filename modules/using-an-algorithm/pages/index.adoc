...
Packaged template queries are a new, simplified method for working with query algorithms that are preinstalled in a TigerGraph instance.
...
xref:gui:graphstudio:overview.adoc[GraphStudio] does not support using the `CALL` command for packaged template queries.
