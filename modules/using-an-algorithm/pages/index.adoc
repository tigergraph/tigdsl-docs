...
The algorithms come pre-installed with TigerGraph version 3.8.0 and later as packaged queries.
On earlier versions of TigerGraph before 3.8.0, and in xref:gui:graphstudio:overview.adoc[GraphStudio], each query must be installed manually.
...
Packaged queries are a new, simplified method for working with query algorithms that are preinstalled in a TigerGraph instance.
The syntax is very similar to using a package in Python.

=== Import and explore packaged queries

Using a TigerGraph installation version 3.8.0 and later, run the following command to import the query package.
...
The sub-packages correspond to a category of GDS algorithm.
...
The first time you call a particular query on a graph, it goes through a short installation process.
This only needs to happen once per query per graph.
...
xref:gui:graphstudio:overview.adoc[GraphStudio] does not support using the `CALL` command for packaged queries.
