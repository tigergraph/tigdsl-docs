 ...
 ** If the first IDin P is also ID(v), a cycle has been found:
  *** Remove P from its list.
  *** If ID(v) is the least IDof any IDin P, then add P to the Cycle List. (The purpose is to count each cycle only once.)
...
Computes a list of vertex IDlists, each of which is a cycle. 
The result is available in 2 forms:
