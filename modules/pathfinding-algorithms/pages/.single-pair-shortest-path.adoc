...
The Single-Pair Shortest Path task seeks the shortest path between a source vertex S and a target vertex T. If the edges are unweighted, then use the query in our tutorial document xref:gsql-ref:tutorials:gsql-examples/classic-graph-algorithms.md#example-8-single-pair-shortest-path-unweighted[GSQL Demo Examples.]
