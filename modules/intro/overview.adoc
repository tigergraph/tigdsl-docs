...
To use an  algorithm, the algorithm (GSQL query) must first be link:../dev/gsql-ref/querying/query-operations.md#install-query[installed]. If your database is on a distributed cluster, you should use the `DISTRIBUTED` option when installing the query to install it in xref:../dev/gsql-ref/querying/distributed-query-mode.adoc[Distributed Query Mode].

Running an algorithm is the same as running a GSQL query. For example, if you selected the JSON option for link:./#pagerank[`pageRank`], you could run it from GSQL as below:
